...
class TraceInfo:    
    ...
    def get2DHistogram(self, size):
        #create the bins...
        distribution = [ [ 0 for x in range(size) ] for y in range(size) ]
        ...
        for x, y in self.points:
            ...
            distribution[v_bin][h_bin] += 1
            """

            h_div = (x + 1.0) / bin_size
            ...
            #print( 'H: ' + str(h_bin0) + '-' + str(h_bin1) + ' , V: ' + str(v_bin0) + '-' + str(v_bin1) + ', S = ' + str(size) )
    ...
    def getGabor(self, grid_size):
        #           ....  0    45  90   135
        distribution = [ 0.0, 0.0, 0.0, 0.0 ] * (grid_size * grid_size)
        ...
        for i in range(len(self.points) - 1):
            ...
            if grid_size >= 2:
                #(0,0)
                distribution[(c0_y *  grid_size + c0_x) * 4 + p0] += g0 * w0_x * w0_y
                distribution[(c0_y *  grid_size + c0_x) * 4 + p1] += g1 * w0_x * w0_y
                #(0,1)
                distribution[(c1_y *  grid_size + c0_x) * 4 + p0] += g0 * w0_x * w1_y
                distribution[(c1_y *  grid_size + c0_x) * 4 + p1] += g1 * w0_x * w1_y
                #(1,0)
                distribution[(c0_y *  grid_size + c1_x) * 4 + p0] += g0 * w1_x * w0_y
                distribution[(c0_y *  grid_size + c1_x) * 4 + p1] += g1 * w1_x * w0_y
                #(1,1)
                distribution[(c1_y *  grid_size + c1_x) * 4 + p0] += g0 * w1_x * w1_y
                distribution[(c1_y *  grid_size + c1_x) * 4 + p1] += g1 * w1_x * w1_y
            else:
                #only 4 values....
                distribution[p0] += g0
                distribution[p1] += g1
    ...
    def getTypeSubsegmentsInfo(self):
        ...
        for init, end, ang, t, l in self.segments:
            ...
            if t == 1:
                ...
                #curve... (THIS IS WAY ERRATIC!!!!)
                distribution_crv[p0] += g0
                distribution_crv[p1] += g1

                #for curves, get distribution of angles for the circumcenter...
                for k in range(init, end - 1):
                    cx, cy, v = self.circumcenter(self.points[k], self.points[k + 1], self.points[k + 2])

                    if v > 0.0:
                        rx, ry = self.points[k + 1]

                        ang = math.atan2( ry - cy, rx - cx )
                        if k == init:
                            #the two lines...
                            arc_length = lengths[k] + lengths[k + 1]
                        else:
                            #only the next second...
                            arc_length = lengths[k + 1]
                        ...
                        #the values....
                        g0 = arc_length * (1.0 - wp1)
                        g1 = arc_length * (wp1)

                        distribution_arc[p0] += g0
                        distribution_arc[p1] += g1
    ...
    def getSubsegmentsFeaturesTypes(self):
        ...
        #return ['v1', 'v1', 'v1'] + ['v1'] * 16 + ['v1'] * 4 + ['v1'] * 4
        #return ['v1'] * 17
        #return ['v1'] + ['v4'] * 4
        #return ['v4'] * 4
        return ['c'] * 8
